...
const Login = ({ navigation, route }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await fetch('http://192.168.65.102:8080/api/auth/login', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        ...
        // Fetch user details
        const userDetailsResponse = await fetch(`http://192.168.65.102:8080/api/users/get-user-details?email=${encodeURIComponent(email)}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
